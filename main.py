...
class MediMapProApp(MDApp):
    ...
    def build(self):
        ...
        if is_mobile:
            ...
            preview_card, preview_body = make_card("Live Preview", "Start with preview, then continue with data and mapping")
            preview_card.size_hint_y = None
            top_action_row = GridLayout(cols=2, spacing=dp(8), size_hint_y=None, height=row_h)
            self.btn_preview = make_button("Refresh", tone="primary")
            self.btn_preview.bind(on_release=self.on_preview)
            self.btn_detect = make_button("Detect", tone="accent")
            self.btn_detect.bind(on_release=self.on_run_detect)
            top_action_row.add_widget(self.btn_preview)
            top_action_row.add_widget(self.btn_detect)
            preview_body.add_widget(top_action_row)
            nav_row = GridLayout(cols=2, spacing=dp(8), size_hint_y=None, height=row_h)
            self.btn_prev = make_button("Prev", tone="plain")
            self.btn_prev.bind(on_release=self.on_prev_page)
            self.btn_next = make_button("Next", tone="plain")
            self.btn_next.bind(on_release=self.on_next_page)
            ...
            mobile_controls_card, mobile_controls_body = make_card("Workspace", "Open files, then move through the steps")
